...
- [ ] T020 [P] [US1] Create `backend/src/services/availabilityService.js` — `findUnavailableItems(sessionId)`: fetch cart items joined with menu_items, return list of unavailable item IDs (is_available=false)
...
- [ ] T027 [P] [US1] Create `frontend/public/cart.html` — cart page: "Orders (N)" button at top linking to orders.html showing count of unpaid orders (hidden when 0); list of current cart items with quantity +/- controls and item name/price; cart total; sticky "Place Order" button at bottom; unavailable items shown grey
- [ ] T028 [US1] Create `frontend/js/cart.js` — Alpine.js `cartApp()`: on init fetch `GET /api/cart?session_id`; on init fetch `GET /api/orders?session_id` and count unpaid orders for `unpaidOrderCount` (drives Orders button visibility); `increment(id)` / `decrement(id)` call PATCH (decrement to 0 = remove); `submitOrder()` calls `POST /api/orders`; on 400 ITEM_UNAVAILABLE highlight unavailable items; on 201 clear items array and navigate to orders.html
...
- [ ] T041 [US4] Mount table routes in `backend/src/app.js`
...
- [ ] T043 [US4] Create `frontend/js/landing.js` — Alpine.js `landingApp()`: parse table_id from URL (if missing show error); fetch `GET /api/table/:tableId/status`; if 404 redirect to error.html; read restaurant_name from the response; if taken store session_id in sessionStorage and show taken message; if free show welcome message with restaurant_name; "Yes"/"To menu" buttons: if free call POST /api/table/:id/session first to get session_id; store session_id in sessionStorage; navigate to menu.html
