...
- **FR-006**: Users MUST be able to review their cart contents and submit an order; immediately after submission the cart MUST be cleared and the user MUST be navigated to the orders page
...
- **FR-009**: The orders page MUST display all orders for the session grouped by payment status: unpaid orders shown first (each with sequential number, time of placement, total, and a "Pay" button), followed by a "Paid" section header, then paid orders (sequential number, date, total — no action buttons); a sticky bar above the bottom "Pay all" button MUST show the total unpaid sum; the "Pay all" button MUST allow the diner to pay all unpaid orders in a single transaction
- **FR-010**: Tapping "Pay" (individual order) or "Pay all" MUST immediately redirect the diner to the external payment provider; after payment succeeds or fails the diner MUST be returned to the orders page with a visible success or failure banner
